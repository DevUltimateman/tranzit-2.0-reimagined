...
main() //checked changed to match cerberus output
{
	...
	registerclientfield( "allplayers", "playerinfog", 1, 1, "int" ); //commenting out will result in exe_clientfield_mismatch
	...
	if ( getDvar( "createfx" ) == "1" ) //Original value 1
	{
		return;
	}
	maps/mp/teams/_teamset_cdc::level_init();
	maps/mp/zm_transit_ai_screecher::init();
	level.is_player_in_screecher_zone = ::hook_is_player_in_screecher_zone; //is_player_in_screecher_zone;
	...
	level.uses_gumps = 1; //original value = 1
	setdvar( "aim_target_fixed_actor_size", 1 );
	setDvar ( "r_fog", 0 );
	//setDvar ( "r_lightTweakSunLight", 15 );
	//setDvar ( "r_lightTweakSunColor", 0, 0.4, 0.9 );
	//setDvar ( "r_lightTweakSunDirection", 300, 250, 12 );
	...
	//setDvar ( "r_lodBiasSkinned", -1000 );
		...
		//sky
	//setDvar ( "r_sky_intensity_factor0", 2 );
	//setDvar ( "r_skyColorTemp", 21440 );
...
#/
	*/
	...
	//init_screecher_zones(); //Screecher zones disabled
	/*
	//This is not in the day 1 patch. Has something to do with either unlimited rounds skip or pile up function
	//Investigate later, comment out for now
...
set_player_in_fog( onoff ) //checked matches cerberus output
{
	if ( onoff )
	{
		self setclientfield( "playerinfog", 0 ); //Original value 1 Ultimateman 2021
	}
	else
	{
		self setclientfield( "playerinfog", 1 ); //Original value 0 Ultimateman 2021
	}
}
*/
...
allow_move_in_laststand( player_down ) //checked matches cerberus output used is_true instead // TEST HACK = test, let player move while in last stand on bus
{
	if ( is_true( player_down.isonbus ) )
	{
		return 1; //original value 0 
	}
	return 1;
}
...
init_screecher_zones() //checked partially changed to match cerberus output see info.md No. 2
{
	foreach ( key in level.zone_keys )
	{
		...
		{
			level.zones[ key ].screecher_zone = 0; //Originally 1
		}
		else
		{
...
is_player_in_screecher_zone( player ) //checked changed to match cerberus output used is_true instead
{
	...
	return 0; //Original value 1 //Ultimateman 2021
}
*/
hook_is_player_in_screecher_zone( player )//CUSTOM OVERRIDE FUNCTION!!! FOR level.is_player_in_screecher_zone // :: hook_is_player_in_screecher_zone;
{
	return 0;
}
...
include_equipment_for_level() //checked matches cerberus output
{
	...
	//limit_equipment( "jetgun_zm", 1 ); //Don't limit jetgun for only 1 player
	//level.explode_overheated_jetgun = 1; //Don't explode jetgun when heat capacity is at
...
offhand_weapon_overrride() //checked matches cerberus output
{
	...
	level.zombie_equipment_player_init = undefined; //undefined // jetgun_zm doesn't work atm
}
