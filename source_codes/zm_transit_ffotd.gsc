...
main_start()
{
	...
	foreach( door in powerdoors ) //Function to fix "invisible" doors when 2 players hold "buy door" button at the same time, might disable later...
	{
...
main_end() //Fixed syntax for vectorscale & collision spawns, will need to comment out later when implementing ffotd_unpatch to the map.
{
	if( is_true( level.optimise_for_splitscreen ) )
	{
		...
		collision4 = spawn( "script_model", ( 12240, 8480, -720 ) ); //Powerstation's electric handle -Ultimateman 2021
		...
		collision11.angles = VectorScale( 0, 0, 1, 180);
		collision11 ghost();
	...
	// Threads disabled. No need to edit their actual function.
	// apartment_exploit();
	// town_truck_exploit();
	// farm_porch_exploit();
	...
	/* Can comment out because no need for deadzone_eqmpnt_fix, fixed syntax to not get errors
...
apartment_exploit() //Will be disabled on thread section when compiling patch
{
...
nacht_exploit() //Will be disabled on thread section when compiling patch
{
...
town_truck_exploit() //Will be disabled on thread section when compiling patch
{
...
farm_porch_exploit() //Will be disabled on thread section when compiling patch
{
...
power_station_exploit() //Will be disabled on thread section when compiling patch
{
...
equipment_safe_to_drop_ffotd( weapon ) //Can change eq_sa_to_dr_ff_ to it's original function on callback thread section on zm_transit.gsc, no need to edit this.
{
...
watch_melee_swipes( weapname ) //Hurt trigger for claymores when trying to avoid zombie attacks by holding clay & facing down
{
